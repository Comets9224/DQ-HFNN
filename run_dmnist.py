"""
Dirty MNIST Dedicated Training Script (Lightweight Version)
- Exclusively supports Dirty MNIST dataset
...
def load_external_params():
    import json
    params_json = os.environ.get('DMNIST_TRAIN_PARAMS')
    if params_json:
        try:
...
my_logger = init_logger("./run_result_dmnist.log", "dmnist_logger")
...
def trainModel(config, run_id=None, batch_timestamp=None):
    """
    Dirty MNIST Dedicated Training Function

    Parameters:
        config: Configuration object (Namespace)
        run_id: Run ID (for multiple executions)
    ...
    if batch_timestamp and run_id:
        ...
        checkpoints_path = os.path.join(config.ckpt_path, 'dmnist', 'joint_membership')
    ...
    my_logger.info(f'Dataset: Dirty MNIST, train_size: {train_data_size:4d}, valid_size: {valid_data_size:4d}')
    ...
    n_random_pairs = getattr(config, 'n_random_pairs', 117)
...
def PredictModel(config):
    """
    Dirty MNIST Dedicated Prediction Function
    ...
    _, data_test = load_dataset_dmnist()
    my_logger.info(f"PredictModel using dataset: Dirty MNIST, run: {run_num}, epoch: {epoch_num}")
    ...
    if batch_timestamp:
        base_path = os.path.join('./checkpoints', 'dmnist', 'joint_membership',
                                 f'batch_{batch_timestamp}', f'run_{run_num}')
    else:
        base_path = os.path.join('./checkpoints', 'dmnist', 'joint_membership', f'run_{run_num}')
    ...
    print("【预测结果评估】")
    print("=" * 60)
    print(f"Accuracy:  {avg_valid_acc * 100:.4f}%")
...
if __name__ == '__main__':
    ...
    base_config = {
        'project_name': 'dmnist_joint_membership',
        'batch_size': 64,
        'lr': 5e-3,
        'optim_type': 'SGD',
        'epochs': 100,
        ...
        'predict_run': 1,
        'predict_epoch': 59,
        'batch_timestamp': '20251023_194507',
    }
