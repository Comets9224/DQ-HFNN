"""
CIFAR-10 Joint Membership Model Dedicated Training Script
- Lightweight version, exclusively supports CIFAR-10 dataset
- Only supports joint_membership model
- Standard training mode (no K-fold)
"""
...
try:
    from models.My_joint_cifar import Cifar10_JointMembership

    print("✓ Loaded: CIFAR-10 joint membership model from My_joint_cifar.py")
except ImportError as e:
    print(f"❌ Failed to import CIFAR-10 model: {e}")
    raise
...
my_logger = init_logger("./run_cifar10_joint.log", "cifar10_joint_logger")
...
def trainModel(config, run_id=None, batch_timestamp=None):
    """
    Train CIFAR-10 Joint Membership Model

    Parameters:
        config: Configuration object
        run_id: Run ID (for multiple runs)
        batch_timestamp: Batch timestamp (for multiple runs)

    Returns:
        model: Trained model
        history: Training history [epoch, 4] (train_loss, valid_loss, train_acc, valid_acc)
    ...
    if batch_timestamp and run_id:
        ...
        checkpoints_path = os.path.join(config.ckpt_path, 'cifar10', 'joint_membership')
    ...
    my_logger.info(
        f'Dataset: CIFAR-10, Model: joint_membership, train_size: {train_data_size:4d}, valid_size: {valid_data_size:4d}')
    ...
    n_random_pairs = getattr(config, 'n_random_pairs', 460)

    model = Cifar10_JointMembership(
    ...
    my_logger.info(f"Using SGD optimizer with lr={LR}, momentum=0.9, weight_decay=1.2e-2")
...
def PredictModel(config):
    ...
    _, data_test = load_dataset_cifar10(
        train_transform=test_transform,
        test_transform=test_transform
    )

    my_logger.info(
        f"PredictModel using dataset: CIFAR-10, model: joint_membership, "
    ...
    if batch_timestamp:
        base_path = os.path.join('./checkpoints', 'cifar10', 'joint_membership',
                                 f'batch_{batch_timestamp}', f'run_{run_num}')
    else:
        base_path = os.path.join('./checkpoints', 'cifar10', 'joint_membership', f'run_{run_num}')
...
if __name__ == '__main__':
    ...
    config = Namespace(
        project_name='cifar10_joint_membership',
        batch_size=128,
        lr=1e-2,
        epochs=100,
        ...
        for run_id in range(1, num_runs + 1):
            ...
            if len(best_acc_list) > 0:
                ...
                my_logger.info(
                    ...
                    f"Best F1={best_f1_list[0]:.4f}%, "
                    f"Time={run_end - run_start:.3f}s")
            ...
            if len(history) == config.epochs:
                ...
                my_logger.warning(
                    f"⚠️  Run {run_id} incomplete: {len(history)}/{config.epochs} epochs")
