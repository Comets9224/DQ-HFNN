...
# ==================== Modification Start: Import models from new file ====================
# Initialize all model variables as None
MyNetwork_cifar10_noent_simply = None  # Two-qubit, single-layer, no entanglement
MyNetwork_cifar10_ent_simply = None  # Two-qubit, single-layer, with entanglement
MyNetwork_cifar10_noent = None  # Two-qubit, double-layer, no entanglement
MyNetwork_cifar10_ent = None  # Two-qubit, double-layer, with entanglement
cifar10_joint_membership = None  # CIFAR-10 joint membership model
DirtyMnist_JointMembership = None  # Dirty MNIST joint membership model

# Import conditional membership model (for CIFAR-10) from My_conditional_model.py
try:
...
# Import the CIFAR-10 joint membership model from My_joint_cifar.py
try:
...
# Import the Dirty MNIST joint membership model from My_joint_dmnist.py
try:
...
# Import the JAFFE joint membership model from My_joint_jaffe.py
try:
...
# Import the Scene15 model from My_joint_scene15.py
try:
    from models.My_joint_scene15_DQHFNN import SCENE15_JointMembership
...
# Keep the conditional membership models for Scene15 and DirtyMNIST as None
...
# ==================== External Parameter Injection Interface ====================
# Used for passing parameters in automated training scripts (via environment variables)
def load_external_params():
    """Read external parameters from environment variables"""
    import json
...
# ==================== End of External Parameter Injection Interface ====================
...
def get_model_and_dataset(config, dataset_type, model_type):
    ...
    if dataset_type == 'scene15':
        ...
        train_transform = transforms.Compose([
            ...
            # transforms.ColorJitter(brightness=0.1, contrast=0.1, saturation=0.1),  # 改为colorJitter,发现无效
            transforms.ToTensor(),
            transforms.Normalize([0.4914, 0.4822, 0.4465], [0.2470, 0.2435, 0.2616]),
            # transforms.RandomErasing(p=0.25, scale=(0.02, 0.15), ratio=(0.3, 3.3), value=0) #弱cutout
        ])
        ...
        # Use the default data augmentation configuration from data_loader.py
        # RandomRotation: Images are randomly rotated within ±10 degrees.
        # RandomAffine (translation): Images are randomly shifted horizontally and vertically by up to 10% of their dimensions.
        # RandomAffine (scaling): Images are randomly scaled between 90% and 110% of their original size.
        data_train, data_test = load_dataset_dmnist()
        num_classes = 10
    elif dataset_type == 'jaffe':
        from data.jaffe.data_loader import load_dataset_jaffe
        # JAFFE only supports K-fold mode
...
def trainModel(config, run_id=None, batch_timestamp=None):
    # Get dataset type and model type
    ...
    # Create separate checkpoint directories for each dataset and model
    # ==================== Modification Start: Support for batch and run numbers ====================
    if batch_timestamp and run_id:
        # Check if it's in K-fold mode (determined by config.fold)
        if hasattr(config, 'fold'):
            # K-fold mode: Each fold has its own directory
            checkpoints_path = os.path.join(
                ...
                f'run_{run_id}',
                # No longer create fold subdirectories
                # f'fold_{config.fold}'
            )
        else:
            # Standard mode: Only run directory exists
            checkpoints_path = os.path.join(
                config.ckpt_path,
                dataset_type,
                model_type,
        ...
        # Backward compatibility with legacy calling methods
    ...
    # Create model instance - unified processing logic
    if model_type == 'joint_membership':
        # Joint membership model: Use different parameters based on dataset
        if dataset_type == 'dmnist':
            ...
            # Scene15-specific parameters (used by DNN/FDNN/QA-HFNN/Dual-Qubit models)
...
if __name__ == '__main__':
    ...
    # ==================== Scene15 Configuration (Supports external parameter overriding) ====================
    base_config = {
        ...
        # Method 2: Directly specify quantities (if not using ratio)
        # 'n_top_bottom_pairs': 30,
        # 'n_top_median_pairs': 20,

        # Method 3: Don't specify (use default 50%-50%)
    ...
    # Merge external parameters (external parameters take precedence)
    if EXTERNAL_PARAMS:
        print(f"\n{'=' * 60}")
    ...
    # ==================== Scene15 Configuration End ====================
    # # JAFFE + K-fold mode
    # config = Namespace(
    #     project_name='2qubit_quantum_nopoolmax',
    #     batch_size=8,
    #     lr=5e-4,  # ✅ Maintain original learning rate
    #     optim_type='AdamW',  # ✅ Upgrade optimizer
    #     warmup_epochs=3,  # ✅ Add mild Warm-up
    #     epochs=100,
    #     ckpt_path='./checkpoints',
    #
    #     dataset='jaffe',
    ...
    #     n_qnn_layers=3,
    #     k=8,
    #     # Random pairs n_random_pairs defaults to 48 (not passed as parameter, unused)
    #     num_runs=5,
    #
    #     #====Prediction parameters==========
    #     predict_run=1,
    ...
    if train_flag:
        ...
        1. Try to get the dataset attribute from the config object
        2. If config.dataset exists, return its value (e.g., 'jaffe', 'dmnist', 'cifar10')
        3. If config.dataset doesn't exist, return the default value 'cifar10'

        If not in K-fold mode (standard mode), k_folds = 1 means training will run only once (no folds)
        """

        # Enable K-fold only for JAFFE when the k parameter is explicitly set
        ...
        # New: Used to store all fold's raw data (only used in K-fold mode)
        if use_kfold:
            all_fold_best_acc = []
            all_fold_best_prec = []
            all_fold_best_rec = []
