...
"""
This file defines models for JAFFE and Dirty-MNIST datasets.

MyNetwork_dmnistn class:
Purpose: A purely classical Dirty-MNIST classifier. It uses MnistModel to extract 128-dim features, then directly classifies into 10 categories using a linear layer self.classi. This serves as a baseline for comparison.

ResidualBlock class: Similar to the Block in CNN_model.py, it's a standard residual block.

MyNetwork_JAFFE_DNN class:
Purpose: The classical feature extractor used in MyNetwork_JAFFE.
Structure: A deeply customized CNN containing multiple convolutional layers, residual blocks, and pooling layers.
Key output: Its forward function ultimately returns self.fc2(x), outputting a 512-dim feature vector. Again, it only extracts features without classification.

MyNetwork_JAFFE class: This is a complete, independent JAFFE classifier, likely serving as a baseline model.
"""
class MyNetwork_dmnistn(nn.Module):
    """ Define the complete neural network architecture """
